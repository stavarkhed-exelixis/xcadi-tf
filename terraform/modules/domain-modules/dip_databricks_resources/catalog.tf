...
resource "databricks_catalog" "this" {
  ...
  name           = "${local.selected_env}-${local.normalized_domain_name}${local.normalized_subdomain_name != "" ? "-${local.normalized_subdomain_name}" : ""}-catalog"
...
resource "databricks_catalog" "additional" {
  ...
  metastore_id   = var.metastore_id
  name           = "${local.selected_env}-${local.normalized_domain_name}${local.normalized_subdomain_name != "" ? "-${local.normalized_subdomain_name}" : ""}-${each.key}-catalog"
...
resource "databricks_workspace_binding" "bind_additional_catalog" {
  for_each       = var.enable_catalog ? local.additional_catalogs_map : {}
  depends_on     = [databricks_catalog.additional]
  provider       = databricks.workspace
  securable_name = databricks_catalog.additional[each.key].name
  securable_type = "catalog"
  workspace_id   = databricks_mws_workspaces.this.workspace_id
}

resource "databricks_grants" "additional_catalog_access" {
  for_each = var.enable_catalog ? local.additional_catalogs_map : {}
  provider = databricks.workspace
  depends_on = [
    databricks_mws_permission_assignment.this,
    databricks_workspace_binding.bind_additional_catalog
  ]

  catalog = databricks_catalog.additional[each.key].name
