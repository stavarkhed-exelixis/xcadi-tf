...
resource "aws_s3_object" "dbx_additional_catalog_folder_creation" {
  for_each = var.enable_catalog ? local.additional_catalogs_map : {}
  bucket   = local.effective_root_storage_bucket
  key      = "unity-catalog/${local.selected_env}/${local.normalized_domain_name}${local.normalized_subdomain_name != "" ? "/${local.normalized_subdomain_name}" : ""}/${each.key}/"
  content  = ""
}
