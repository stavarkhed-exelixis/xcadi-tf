locals {
  ...
  ext_catalog_role_name            = "${local.selected_env}-${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-${var.external_catalog_iam_role_name_suffix}"

  ext_catalog_single_prefix = var.external_catalog_is_custom_path && trimspace(var.external_catalog_custom_bucket_path) != "" ? var.external_catalog_custom_bucket_path : "${var.domain_name}${var.team_name != "" ? "/${var.team_name}" : ""}"
  ...
  ext_catalog_bootstrap_key = var.external_catalog_is_custom_path ? "${var.external_catalog_custom_bucket_path}/" : "${var.domain_name}/${var.team_name != "" ? "${var.team_name}/" : ""}"
}
...
resource "aws_iam_role_policy" "dbx_external_catalog_access" {
  ...
  name = "${local.selected_env}-${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-dbx-ext-catalog-rw-policy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = ["arn:aws:secretsmanager:${var.aws_region}:${local.env_account_id}:secret:${local.selected_env}-${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-*"]
      },
      {
...
resource "aws_s3_object" "dbx_team_onboarding_folder_creation" {
  for_each = local.create_external_catalog_iam_role && var.enable_external_catalog_s3_prefix_creation ? local.ext_catalog_bucket_map : {}

  bucket  = each.value
  key     = local.ext_catalog_bootstrap_key
  content = ""
}
