locals {
  ...
  derived_external_catalog_iam_role_arn = "arn:aws:iam::${local.env_account_id}:role/${local.selected_env}-${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-${var.external_catalog_iam_role_name_suffix}"
...
resource "databricks_storage_credential" "dbx_storage_credential" {
  ...
  name    = "${local.selected_env}-${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-dbx-storage-credential"
  comment = "Credential for ${local.selected_env}_${var.domain_name}${var.team_name != "" ? "_${var.team_name}" : ""} external locations"
...
resource "databricks_credential" "dbx_service_credential" {
  ...
  name    = "${local.selected_env}-${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-dbx-service-credential"
  comment = "Service credential for ${local.selected_env}-${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}"
  purpose = "SERVICE"
...
resource "databricks_external_location" "raw_dbx_external_location" {
  ...
  name            = "${local.selected_env}-${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-dbx-raw-external-location"
  url             = "s3://${var.raw_external_catalog_s3_url}"
  credential_name = databricks_storage_credential.dbx_storage_credential[0].name
  comment         = "RAW external location for ${local.selected_env}_${var.domain_name}${var.team_name != "" ? "_${var.team_name}" : ""} data"
  isolation_mode  = "ISOLATION_MODE_ISOLATED"
}
...
resource "databricks_external_location" "additional_raw_dbx_external_location" {
  ...
  name = "${local.selected_env}-${var.domain_name}${local.team_suffix}-dbx-raw-${each.key}-external-location"

  url             = "s3://${local.effective_additional_external_location_raw_bucket}/${each.value}"
  credential_name = databricks_storage_credential.dbx_storage_credential[0].name
  comment         = "RAW external location for ${local.selected_env}_${var.domain_name}${var.team_name != "" ? "_${var.team_name}" : ""} at prefix '${each.value}'"
  isolation_mode  = "ISOLATION_MODE_ISOLATED"
}
...
resource "databricks_external_location" "staging_dbx_external_location" {
  ...
  name            = "${local.selected_env}-${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-dbx-staging-external-location"
  url             = "s3://${var.staging_external_catalog_s3_url}"
  credential_name = databricks_storage_credential.dbx_storage_credential[0].name
  comment         = "STAGING external location for ${local.selected_env}_${var.domain_name}${var.team_name != "" ? "_${var.team_name}" : ""} data"
  isolation_mode  = "ISOLATION_MODE_ISOLATED"
}
...
resource "databricks_external_location" "additional_staging_dbx_external_location" {
  ...
  name = "${local.selected_env}-${var.domain_name}${local.team_suffix}-dbx-staging-${each.key}-external-location"

  url             = "s3://${local.effective_additional_external_location_staging_bucket}/${each.value}"
  credential_name = databricks_storage_credential.dbx_storage_credential[0].name
  comment         = "STAGING external location for ${local.selected_env}_${var.domain_name}${var.team_name != "" ? "_${var.team_name}" : ""} at prefix '${each.value}'"
  isolation_mode  = "ISOLATION_MODE_ISOLATED"
}
...
resource "databricks_external_location" "analytics_dbx_external_location" {
  ...
  name            = "${local.selected_env}-${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-dbx-analytics-external-location"
  url             = "s3://${var.analytics_external_catalog_s3_url}"
  credential_name = databricks_storage_credential.dbx_storage_credential[0].name
  comment         = "ANALYTICS external location for ${local.selected_env}_${var.domain_name}${var.team_name != "" ? "_${var.team_name}" : ""} data"
  isolation_mode  = "ISOLATION_MODE_ISOLATED"
}
...
resource "databricks_external_location" "additional_analytics_dbx_external_location" {
  ...
  name = "${local.selected_env}-${var.domain_name}${local.team_suffix}-dbx-analytics-${each.key}-external-location"

  url             = "s3://${local.effective_additional_external_location_analytics_bucket}/${each.value}"
  credential_name = databricks_storage_credential.dbx_storage_credential[0].name
  comment         = "ANALYTICS external location for ${local.selected_env}_${var.domain_name}${var.team_name != "" ? "_${var.team_name}" : ""} at prefix '${each.value}'"
  isolation_mode  = "ISOLATION_MODE_ISOLATED"
}
