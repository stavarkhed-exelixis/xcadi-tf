resource "databricks_service_principal" "svc_atlan" {
  ...
  display_name = "${local.selected_env}_${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}_svc_atlan"
}
...
resource "aws_secretsmanager_secret" "databricks_atlan_sp_secret" {
  ...
  name        = "${local.selected_env}_${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-atlan-dbx-svc-principal"
  description = "${local.selected_env}_${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""} Databricks atlan service principal credentials"
}
