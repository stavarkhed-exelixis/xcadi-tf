resource "databricks_service_principal" "svc_sas" {
  ...
  display_name = "${local.selected_env}_${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}_svc_sas"
}
...
resource "aws_secretsmanager_secret" "databricks_sas_sp_secret" {
  ...
  name        = "${local.selected_env}_${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-sas-dbx-svc-principal"
  description = "${local.selected_env}_${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""} Databricks sas service principal credentials"
}
