...
resource "databricks_catalog" "this" {
  ...
  provider     = databricks.workspace
  storage_root = "s3://${local.effective_root_storage_bucket}/unity-catalog/${local.selected_env}/${var.domain_name}${var.team_name != "" ? "/${var.team_name}/" : ""}"
  metastore_id = var.metastore_id
  name         = "${local.selected_env}-${var.domain_name}-catalog"
