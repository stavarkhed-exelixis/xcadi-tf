resource "databricks_service_principal" "svc_dbt" {
  ...
  display_name = "${local.selected_env}_${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}_svc_dbt"
}
...
resource "aws_secretsmanager_secret" "databricks_dbt_sp_secret" {
  ...
  name        = "${local.selected_env}_${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-dbt-dbx-svc-principal"
  description = "${local.selected_env}_${var.domain_name}${var.team_name != "" ? "-${var.team_name}" : ""} Databricks dbt service principal credentials"
}
