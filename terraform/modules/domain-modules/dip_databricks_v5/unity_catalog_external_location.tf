...
resource "databricks_storage_credential" "dbx_root_storage_credential" {
  ...
  name    = "${local.selected_env}-${local.normalized_domain_name}${var.team_name != "" ? "-${var.team_name}" : ""}-root-dbx-uc-storage_credential"
  comment = "Credential for root ${local.selected_env}_${local.normalized_domain_name}${var.team_name != "" ? "_${var.team_name}" : ""} external location"
  aws_iam_role {
