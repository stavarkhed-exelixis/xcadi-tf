...
output "target_backend_irsa_role_arn" {
  description = "Resolved target-account role ARN assumed by this module for AWS resource operations."
  value       = local.target_backend_irsa_role_arn
}

output "selected_account_supported_environments" {
  description = "Allowed env values for the selected account_number. Useful for UI dropdown filtering."
  value       = local.selected_account_supported_envs
}
